...
export abstract class ArcherRouter {
  ...
  public static swapCallParameters(
    ...
    const etherIn = trade.inputAmount.currency === MINTME.onChain(ChainId.MINTME)
    const etherOut = trade.outputAmount.currency === MINTME.onChain(ChainId.MINTME)
    // the router does not support both ether in and out
    invariant(!(etherIn && etherOut), 'ETHER_IN_OUT')
    invariant(!('ttl' in options) || options.ttl > 0, 'TTL')
    invariant('ethTip' in options && options.ethTip?.currency === MINTME.onChain(ChainId.MINTME))
