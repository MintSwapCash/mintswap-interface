...
import { BAR_ADDRESS, ZERO } from '@mintswapcash/sdk'
import React, { useEffect, useState } from 'react'
import { MINT, XMINT } from '../../config/tokens'
...
import { getDayData, useMintPrice } from '../../services/graph'
...
const fetcher = (query) => request('https://thegraph.mintswap.cash/subgraphs/name/mintswap/bar', query)

export default function Stake() {
  const { i18n } = useLingui()
  const { account, chainId } = useActiveWeb3React()
  const mintBalance = useTokenBalance(account ?? undefined, MINT[chainId])
  const xMintBalance = useTokenBalance(account ?? undefined, XMINT[chainId])

  const mintPrice = useMintPrice()

  const { enter, leave } = useSushiBar()

  const { data } = useSWR(`{bar(id: "0x584cd161a2263b62b5d3d267440328b7d2abf319") {ratio, totalSupply}}`, fetcher)
  ...
  const balance = activeTab === 0 ? mintBalance : xMintBalance
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const apr = (((results[1].volumeUSD * 0.05) / data?.bar?.totalSupply) * 365) / (data?.bar?.ratio * mintPrice)
      setApr(apr)
    }
    fetchData()
  }, [data?.bar?.ratio, data?.bar?.totalSupply, mintPrice])

  return (
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake | Mint</title>
        <meta
          key="description"
          name="description"
          content="Stake MINT in return for xMINT, an interest bearing and fungible ERC20 token designed to share revenue generated by all MINT products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://app.mintswap.cash/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE MINT" />
        <meta
          key="twitter:description"
          name="twitter:description"
          content="Stake MINT in return for xMINT, an interest bearing and fungible ERC20 token designed to share revenue generated by all MINT products."
        />
        <meta key="twitter:image" name="twitter:image" content="https://app.mintswap.cash/xmist-sign.png" />
        <meta key="og:title" property="og:title" content="STAKE MIST" />
        <meta key="og:url" property="og:url" content="https://app.mintswap.cash/stake" />
        <meta key="og:image" property="og:image" content="https://app.mintswap.cash/xmist-sign.png" />
        <meta
          key="og:description"
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking MINT for xMINT`)}
              </div>
              {/* <div className="self-start pl-6 pr-3 mb-1 min-w-max md:hidden">
                                <img src={XSushiSignSmall} alt="xmint sign" />
                            </div> */}
            </div>
            <div className="max-w-lg pr-3 mb-2 text-sm leading-5 text-gray-500 md:text-base md:mb-4 md:pr-0">
              {i18n._(t`For every swap on the exchange on every chain, 0.05% of the swap fees are distributed as MINT
                                proportional to your share of the MintSwap Bar. When your MINT is staked into the MintSwap Bar, you receive
                                xMINT in return.
                                Your xMINT is continuously compounding, when you unstake you will receive all the originally deposited
                                MINT and any additional from fees.`)}
            </div>
            {/* <div className="flex">
                            ...
                            <div>
                                <StyledLink className="text-sm text-lg whitespace-nowrap md:text-lg md:leading-5">
                                    Tips for using xMINT
                                </StyledLink>
                            </div>
                        </div> */}
          </div>
          <div className="hidden px-8 ml-6 md:block w-72">
            <Image src="/xmint-sign.png" alt="xMINT sign" width="100%" height="100%" layout="responsive" />
          </div>
        </div>
            ...
            <div className="mb-4">
              {
                <div className="flex items-center justify-between w-full h-24 max-w-xl p-4 rounded md:pl-5 md:pr-7 bg-light-yellow bg-opacity-40">
                  <div className="flex flex-col">
                    ...
                    <div className="flex">
                      <a
                        href={`https://analytics.mintswap.cash/bar`}
                        target="_blank"
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake MINT`) : i18n._(t`Unstake`)}
                  </p>
                  <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xMIST = ${xSushiPerSushi.toFixed(4)} MINT`}
                  </div>
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}MINT`}
                      </p>
                    </div>
              ...
              <div className="flex flex-wrap">
                <div className="flex flex-col flex-grow md:mb-14">
                  ...
                  <div className="flex items-center space-x-4">
                    <Image
                      ...
                      src="/images/tokens/xmint-square.jpg"
                      alt="xMINT"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {xMintBalance ? xMintBalance.toSignificant(8) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">xMIST</p>
                    </div>
                  </div>
                  {(xMintBalance && xSushiPerSushi) ?
                    (<div className="mt-3">
                      ~ {xMintBalance.multiply(Math.round(xSushiPerSushi * 1e8)).divide(1e8).toSignificant(8)} MINT
                    </div>) : (<></>)
                  }
                </div>
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    <Image
                      className="max-w-10 md:max-w-16 -ml-1 mr-1 md:mr-2 -mb-1.5 rounded"
                      src="/images/tokens/mint-square.jpg"
                      alt="MINT"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {mintBalance ? mintBalance.toSignificant(8) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">MINT</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col w-full mb-4 mt-7 md:mb-0">
                  {account && (
                    <a
                      href={`https://analytics.mintswap.cash/users/${account}`}
                      target="_blank"
                      rel="noreferrer noopener"
                      className={`
                            ...
                            `}
                    >
                      {i18n._(t`Your MinSwap Bar Stats`)}
                    </a>
                  )}
                </div>
                */}
