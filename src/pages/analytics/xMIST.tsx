import { MINT_ADDRESS } from '@mintswapcash/sdk'
...
import { XMINT } from '../../config/tokens'
import useActiveWeb3React from '../../hooks/useActiveWeb3React'

export default function XMINT_PAGE() {
  ...
  const xSushi = useTokens({ chainId, subset: [XMINT[chainId].address] })?.[0]
  const xSushi1d = useTokens({ block: block1d, chainId, subset: [XMINT[chainId].address] })?.[0]
  const sushiDayData = useTokenDayData({ token: MINT_ADDRESS[chainId], chainId })
  ...
  const graphs = useMemo(
    () => [
      ...
      {
        labels: ['Mint Staked (USD)', 'Mint Harvested (USD)'],
        note: '/ day',
        data: [
          data.map((d) => ({
            date: d.date * 1000,
    ...
    <AnalyticsContainer>
      <Background background="bar">
        <div className="grid items-center justify-between grid-cols-1 gap-x-4 gap-y-2 md:grid-cols-2">
          <div className="space-y-5">
            <div className="text-3xl font-bold text-high-emphesis">xMint</div>
            <div>Find out all about xMint here.</div>
          </div>
      ...
      <div className="pt-4 space-y-5 lg:px-14">
        <div className="flex flex-row space-x-4 overflow-auto">
          ...
          <InfoCard text="xMINT Supply" number={formatNumber(bar?.totalSupply)} />
          <InfoCard text="xMINT : MINT" number={Number(bar?.ratio ?? 0)?.toFixed(4)} />
        </div>
